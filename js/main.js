	...
	$.ajax({
		...
		'success': function(response){
			...
			for(var i = 0; i < responseLength; i++)
			{
				...
				var timeOfEvent = cleanDate.substring(12,20);

				var data = "<li date='" + response.results[i].dateOfShow + "'><div class='row idfk'><div class='large-8 columns'><span class='title'>" +
				 response.results[i].eventDateName + "</span><br><span class='idk'>" +
				 response.results[i].name + "</span><br><span class='idk'>" +
				 response.results[i].eventHallName + "</span><br><span class='date'>" +
				 cutDate + " kl:" + timeOfEvent + "</span><span class='group'>" + response.results[i].userGroupName + 
				 "</span></div><div class='large-4 columns'><img src='" + response.results[i].imageSource + "'></div></div></li>";
			...
			for(var i = 0; i < responseLength; i++)
			{
				...
				if (new Date(cutDate) >= new Date(dateFrom) && new Date(cutDate) <= new Date(dateTo)) {
					var data = "<li date='" + response.results[i].dateOfShow + "'><div class='row idfk'><div class='large-8 columns'><span class='title'>" +
					 ...
					 response.results[i].name + "</span><br><span class='idk'>" +
					 response.results[i].eventHallName + "</span><br><span class='date'>" +
					 cutDate + "</span><span class='group'>" + response.results[i].userGroupName + 
					 "</span></div><div class='large-4 columns'><img src='" + response.results[i].imageSource + "'></div></div></li>";
					}
